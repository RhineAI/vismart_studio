                ...
                <table class="table table-bordered table-client">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="text-center table-danger" style="color:black;">Logo</th>
                    ...
                    <tbody>
                        @foreach ($client as $item => $key)
                        <tr>
                            <th class=" table-secondary">{{ $item+1 }}</th>
                            <th class=" table-secondary">{{ $key->name }}</th>
                            <th class=" table-secondary">{{ $key->description }}</th>
                            <th class="table-secondary">
                                <a href="/dashboard/client/{{ $key->id }}/edit" class="btn btn-xs bg-info"><span data-feather='edit'></span></a>
                                
                                <form action="{{ route('client.destroy', $key->id) }}" method="post" class="d-inline">
                                    @method('delete')
                                    @csrf
